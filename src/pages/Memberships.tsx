  ...
  useEffect(() => {
    const loadUserMembership = async () => {
      try {
        ...
        // Obtener el nombre del plan de membresía
        let title = '';
        if (umRow.membership_plan_id) {
          const { data: plan } = await supabase
            .from('membership_plans')
            .select('name')
            .eq('id', umRow.membership_plan_id)
            .maybeSingle();
          if (plan?.name) title = plan.name;
        }
    ...
    <>
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
        <div className="text-center py-3 sm:py-6 md:py-8 px-2">
          ...
          <h2 className="text-xl sm:text-2xl font-bold mb-1.5 sm:mb-2">Elige tu plan ideal</h2>
          ...
          {!loadingMemberships && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {memberships.map((membership) => (
                <Card
                  key={membership.id}
                  className={`h-full flex flex-col transition-all duration-300 ${userMembership && userMembership.membershipId === membership.id ? 'border-success bg-success/10 shadow-soft' : 'shadow-soft border border-border'}`}
                >
                  <CardHeader className="text-center pb-2 sm:pb-3 px-4 sm:px-6">
                    ...
                    <CardTitle className="text-base sm:text-lg">{membership.title}</CardTitle>
                  ...
                  <CardContent className="flex flex-col flex-grow px-4 sm:px-6 pb-4 sm:pb-6">
                    ...
                    {userMembership && userMembership.membershipId === membership.id ? (
                      <Button className="w-full mt-auto bg-success text-white h-10 text-sm" size="lg" disabled>
                        Membresía activa
                      </Button>
                    ) : (
                      <Button
